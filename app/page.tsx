    ...
    <div className="flex h-screen bg-black text-white selection:bg-white selection:text-black">
      ...
      <aside className="w-72 bg-black border-r border-[#222] p-8 flex flex-col">
        ...
        <nav className="flex-1 space-y-2">
          <button onClick={() => setSelectedFolder(null)} className={`w-full text-left px-4 py-2.5 rounded-lg text-sm transition ${!selectedFolder ? 'bg-[#111] border border-[#333]' : 'text-[#888] hover:text-white'}`}>
            Global Dashboard
          </button>
          
          <div className="pt-8 pb-3 text-[10px] font-bold text-[#444] uppercase tracking-widest">Collections</div>
          {folders.map(folder => (
            <button key={folder.id} onClick={() => setSelectedFolder(folder.id)} className={`w-full text-left px-4 py-2 rounded-lg text-sm flex items-center justify-between transition ${selectedFolder === folder.id ? 'text-white font-bold' : 'text-[#888] hover:text-white'}`}>
      ...
      <main className="flex-1 overflow-y-auto p-16">
        ...
        <section className="bg-[#0a0a0a] border border-[#1a1a1a] rounded-3xl p-12 mb-20 shadow-2xl">
          ...
          <div className="mt-8 flex items-center gap-3">
            ...
            <label htmlFor="pvis" className="text-[10px] font-bold text-[#666] uppercase tracking-widest cursor-pointer">File Visibility: {isPublic ? 'Public' : 'Restricted'}</label>
          </div>
        </section>
